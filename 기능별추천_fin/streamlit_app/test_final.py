...
if st.session_state.page == "검진 기반 추천":
    ...
    if st.button("추천 요청"):
        ...
        with st.spinner(random.choice(loading_messages)):
            payload = {
                "body_part": body_part,
                "function":  user_input.strip()
            }
            # 1) 기능 매칭
            match_json = call_api("POST", "/bodypart/bodyfunction/match", json=payload)
            if match_json is None:
                st.stop()

            matched_fn = match_json.get("matched_function", user_input.strip())
            ...
            rec_json = call_api("POST", "/bodypart/recommend", json=rec_payload)
            if rec_json is None:
                st.stop()

        # ───── 결과 렌더링 ─────
        st.success("✅ 추천 결과")
        st.markdown(rec_json.get("recommendation", "결과 없음"))

        # 예시로 매칭된 제품 표
        prods = rec_json.get("matched_supplements", [])
        if prods:
            st.dataframe(prods, hide_index=True, use_container_width=True)

        # 컨텍스트(근거) 확인용 토글
        if st.toggle("🔍 RAG 컨텍스트 보기"):
            st.write(rec_json.get("context", []))
